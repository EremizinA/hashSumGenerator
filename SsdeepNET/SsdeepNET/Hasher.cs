...
namespace SsdeepNET
{
    public sealed class Hasher
    {
        ...
        /// </summary>
        public static string HashBuffer(string buf, int len, FuzzyHashMode flags = FuzzyHashMode.None)
        {
            var ctx = new Hasher();
            ctx.Update(buf, len);
        ...
        /// </summary>
        private void Update(string buffer, int len)
        {
            _totalSize += (uint)buffer.Length;

            for (int i = 0; i < len; i++)
                this.EngineStep((byte)(buffer[i]));
        }
    }
}